...
Nodes can be connected to each other. They can have input nodes on their left side and output nodes on their right side. Usually there is one input for a chain of filters.
...
- Download the provided *Graph-ICS_X.X* binary for your platform and run the setup.
...
You can open and save a configuration of nodes via the *File* menu. All configuration will be saved. Including the view connection a parameter.
...
Create nodes in the same manner as using the Searchpanel
...
- You have successfully build OpenCV from the source code!
...
- Extract the the contents from the archive into the directory where you want to store the ITK library (e.g. *C:/lib/itk/*, */home/user/lib/itk/*)
...
- Open Qt Creator, go to *Tools* and click *Options*:
- Select *Kits*.
...
The structure of filter classes is quite similar. You can orient on predefined filters. 
...
- Pay attention to the following steps:
    ...
    (4) retrieveResult() is a pure virtual function of the base class. It should be implemented, and it shall contain the actual filter functionality.
